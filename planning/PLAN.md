...
- ~~Manual reassign/unassign endpoints~~ — built in Phase 1 (see above). ~~Off-duty/zone soft-constraint warnings~~ — built in Phase 2 (see above). Remaining here: wiring `manual` assignments to be fully excluded from auto-re-dispatch/rebalancing once a rebalancer exists — Phase 2's `assigner` guards against creating a *second* active assignment for an already-assigned order (closing the most direct double-dispatch risk), but doesn't yet special-case `source: "manual"` assignments the way a future rebalancer will need to.
