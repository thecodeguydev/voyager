...
6. **Pipeline Editor** — visual, drag-to-reorder stage builder with per-stage config; preset picker; live preview. For a jurisdiction with no `pipeline_configs` row yet (the engine is silently running Phase 2's fallback — a single Scoring stage with Settings-resolved weights), the editor surfaces an explicit "initialize" / "restore to preset" action (default: `balanced`) rather than presenting an empty builder — this is how a jurisdiction opts into the new pipeline explicitly, per the Phase 3 decision to keep that fallback silent and behavior-preserving until acted on.
...
├── interface/  # Next.js 16 (App Router) — dashboard, telemetry, editors, settings
│   └── app/{dashboard,telemetry,orders,workers,groups,jurisdictions,settings,metrics}/ · components/{ui,dashboard,telemetry,pipeline}/ · lib/{api,hooks,scope,types,statusTone,format,timeWindow}.ts(x)
├── shared/     # Shared Sequelize models + SettingsService (used by api + engine)
├── scripts/    # windows_start/stop.ps1, linux_start/stop.sh — bring up Postgres + all three services
...
**Implementation notes (for later phases):**
- **New `engine/` workspace.** `engine/src/{config,listener,consumer,resolver,matcher,assigner,heartbeat,settingsCache,telemetry,index}.ts` + `engine/src/pipeline/{stage,runner,scoringStage}.ts`. Mirrors `api/`'s package.json/tsconfig/vitest conventions exactly (native ESM, `NodeNext`, same dependency pins). `queue-consumer`'s `LISTEN dispatch_new` client (`listener.ts`) and the poll-interval safety net both call the same `createQueueRunner()` wake-coalescing drain loop in `consumer.ts` — a wake that arrives mid-cycle triggers one more full drain after, rather than being dropped.
- **The `dispatch_queue` "done" update lives inside `assigner.ts`'s transaction**, not as a separate post-assignment step — a failure after that point (e.g. telemetry) can no longer roll a successfully-assigned row back to `pending` and cause a double-dispatch. `assigner.ts` also re-checks under lock that the order doesn't already have an active assignment (defense in depth against a race with a concurrent manual reassign), before locking the candidate worker row.
- **Key Decision #3 (shared owns cross-service logic) extended**: `shared/src/dispatch/{constants,capacity,lifecycle,enqueue,scheduleWindow,eligibility}.ts` and `shared/src/db/createDb.ts` (moved out of `api/src/db.ts`, which is now a thin re-export) are the single source of truth for state-set constants, the capacity cascade, the assignment state machine, dispatch_queue re-enqueue, and the schedule-window SQL fragments — used by both `api/` and `engine/` so they can't silently diverge. `engine/src/matcher.ts` and the new `shared/src/dispatch/eligibility.ts` (used by `assignmentService.reassignOrder`'s off-duty/zone warnings) share the same `withinScheduleWindow`/`onScheduledDay` SQL-fragment builders.
- **Schedule time-window matching handles overnight wraparound.** A plain `BETWEEN startTime AND endTime` can never match a shift like 22:00–06:00 (Postgres requires start <= end); `withinScheduleWindow()` adds an OR branch for `startTime > endTime`. Day-of-week attribution for a wrapped shift still keys off the *start* day, not a finer-grained "which half of the wraparound is it" distinction — a known simplification, not a full solve.
- **Settings-cache built now, not deferred** — `engine/src/settingsCache.ts` caches per-jurisdiction Scoring weights, invalidating by comparing `jurisdictions.settingsVersion` on every `.get()` call (a cheap indexed PK lookup, per the plan's "cheaply checks settingsVersion each cycle"). Resolution always falls back to an in-code default (`DEFAULT_SCORING_WEIGHTS`) if no setting is found at any scope, so the pipeline is never dependent on the seed migration having run.
- **`metric_definitions`/`metric_points` pulled forward** (migrations 0016-0019) with 2 seeded built-ins (`dispatch.response_time_ms`, `dispatch.time_to_assign_ms`) — no partitioning yet (Phase 4). `engine_instances` also added (migration 0016), standalone, no FKs.
- **`pipeline.scoring.weights.*` seeded as global settings** (migration 0019) so jurisdictions can already override Scoring weights before `pipeline_configs` (Phase 3) exists — resolved the same way `worker.max_concurrent` is.
- **The Scoring stage's `waitTime` component is per-candidate idle time** (`worker.updatedAt`, a proxy since there's no explicit idle-since tracking), not per-order wait time — a per-order value is identical across every candidate for one dispatch and can never affect ranking, so it would have been a dead weight.
- Migrations must not import from `shared/src` — Umzug loads migration files via its own dynamic `import()`, which isn't routed through a TS-aware loader the way the rest of `shared/src` is when run through `vitest`/`tsx`. Migration 0019 originally tried `initModels()` + `Setting.bulkCreate()` and failed at runtime with "Cannot find module .../models/index.js"; fixed by using plain `queryInterface.bulkInsert` like every other migration (JSONB values passed as `JSON.stringify()`'d text, which Postgres implicitly casts).
- Building a Sequelize model instance from a raw-query row via `Model.build(rawRow, { isNewRecord: false })` silently drops `createdAt`/`updatedAt` — not a safe hydration path. `matcher.ts` selects only `id` + computed figures from raw SQL, then fetches real `Worker` instances via `findAll({ where: { id: [...] } })`.

**Phase 3 — Composable pipeline & manual override** ✅ Done
- TierFilter + Tiebreak stages; `pipeline_configs`; presets; reorder/toggle; `pipelineTrace`.
- ~~Manual reassign/unassign endpoints~~ — built in Phase 1 (see above). ~~Off-duty/zone soft-constraint warnings~~ — built in Phase 2 (see above). ~~Wiring `manual` assignments to be fully excluded from auto-re-dispatch/rebalancing~~ — the shared predicate (`isManualAssignment`) is built now (see below); the rebalancer itself is still Phase 4.
- ~~Settings hot-reload via `settingsVersion`~~ — built in Phase 2 (see above), including the engine-side cache/reload, ahead of this doc's original phase placement.

**Implementation notes (for later phases):**
- **New tables/models:** `pipeline_configs` (migration 0020 — `jurisdictionId` unique FK, `preset`, `stages` JSONB, `enabled`; no seed rows) plus `shared/src/models/PipelineConfig.ts`. `AuditLog`'s `entity` enum already included `"pipeline_config"` since Phase 1, so no audit schema change was needed.
- **Stage config validation lives once, in `shared/src/dispatch/pipelineStages.ts`** — a Zod discriminated union (`tier`/`scoring`/`tiebreak`) both `api` (validates `PUT /jurisdictions/:jid/pipeline` bodies) and `engine` (re-validates a stored row before building stages, defense against a hand-edited row) import, so the two can't diverge on what a valid stage document looks like. `shared/src/dispatch/pipelinePresets.ts` holds the fixed `simple`/`balanced`/`advanced` stage catalogs (`custom` has no fixed content); `DEFAULT_SCORING_WEIGHTS` moved from `engine/src/pipeline/scoringStage.ts` into shared so the preset catalog and the engine's fallback path share one constant instead of two copies.
- **The engine's runtime fallback for a jurisdiction with no `pipeline_configs` row (or a disabled/schema-invalid one) is deliberately unchanged from Phase 2** — a single Scoring stage with weights resolved through the existing Settings cascade — so adopting the new pipeline is an explicit per-jurisdiction action (via `PUT`), never a silent behavior change for a jurisdiction already running. Phase 5's Pipeline Editor must surface an explicit "initialize"/"restore to preset" action for this empty-config case rather than a blank builder (noted inline at Phase 5's Pipeline Editor bullet below).
- **TierFilter tags, it does not gate or persist.** It resolves a tier (explicit `order.priorityTier`, else walking `config.tiers` against a minutes-until-`slaDueAt` cutoff in `config.sla`, else the last configured tier) and records it only in `candidate.trace.tier` (which flows into the winning assignment's `pipelineTrace` for Dispatch Telemetry). It deliberately does **not** write the resolved tier back onto `order.priorityTier`: an earlier draft did, and a review caught that this would make a pipeline-computed tier indistinguishable from a caller-supplied one on the order's next dispatch attempt (e.g. after a reject/expire re-queue), permanently freezing a stale tier instead of re-running the SLA computation. Hard eligibility gating by tier (PLAN.md's "can gate which workers are eligible for high tiers") is deferred until a jurisdiction has a concrete need for it — Scoring's `skillMatch` already provides soft skill-based ranking.
- **Tiebreak groups candidates within a small score epsilon of each other**, using the tied group's first (highest-scoring) member as the comparison anchor (not the previous element) so a chain of small steps can't drift a group's range apart. `fifo` is a stable pass-through (the candidates' natural matcher order — there's no cross-order queue to draw "first" from at this point in the pipeline); `nearest` sorts a tied group by ascending distance; `round_robin` sorts by each worker's most recent `dispatchedAt` ascending (never-dispatched first) — derived statelessly from `assignments`, no rotation-state column, and only queried when a `round_robin` tie group of size > 1 actually exists.
- **`isManualAssignment()` (`shared/src/dispatch/manualAssignment.ts`)** is the one Phase 3 deliverable for the manual-override-exclusion item — a small, unit-tested predicate with no caller yet (Phase 4's scheduler is its first consumer). A paired `AUTO_REBALANCE_ASSIGNMENT_SOURCES` constant was considered and dropped: it encoded the same two-value fact a second way with no current consumer of its own.
- **`pipelineConfigService.ts` lives in `api/src/services/`, not `shared/`** — mirrors `assignmentService.ts`'s precedent: the engine only ever reads `PipelineConfig` directly (a plain `findOne` in `settingsCache.ts`, no cascade logic needed since `jurisdictionId` is unique), so the transactional upsert + audit + `settingsVersion`-bump logic is API-only, same as manual reassignment never routing through a shared service either.

**Phase 4 — Telemetry & metric dictionary** ✅ Done
- Full built-in metric seed (12 total — the 2 from Phase 2 plus 10 new); `metric_points` emission from both `api` (lifecycle/manual-override/order-creation events) and `engine` (dispatch/expiry events and periodic gauges); `GET /metrics/query` aggregation; `GET`/`POST /metrics/definitions` for custom metrics.
- `metric_points` converted to native declarative range partitioning by `ts` (monthly); the engine scheduler keeps 3 months of future partitions pre-created and drops partitions past the retention window (a resolved global setting, default 90 days).
- SLA-expiry sweep scheduler. **Workload rebalancing is explicitly deferred** — the product owner confirmed this phase ships expiry-sweep + partition-maintenance only, since PLAN.md never specified a concrete rebalance algorithm; `isManualAssignment()` (built in Phase 3) still awaits its first caller.

**Implementation notes (for later phases):**
- **Two pre-existing gaps closed, not scope creep:** `engine/src/assigner.ts` never set `expiresAt` on a new assignment, and `api/src/services/lifecycleService.ts`'s `applyLifecycleEvent` never set `respondedAt`/`completedAt` even though both columns existed on `Assignment` since Phase 1. Both are required for this phase's SLA sweep and `assignment.duration_ms`/`sla.compliance_rate` metrics, so they're fixed here rather than deferred further.
- **New global settings** (migration 0021, combined with the metric seed like migration 0019's precedent): `assignment.response_timeout_ms` (default 300000ms — how long a dispatched assignment, auto or manual, waits for a worker response before the scheduler expires it) and `metrics.retention_days` (default 90). Both resolved through the existing Settings cascade (`shared/src/dispatch/responseTimeout.ts`), so either can be overridden per group/jurisdiction like any other setting.
- **`emitMetric` and `recordAssignmentAudit` moved from `engine`/`api` into `shared/`** (Key Design Decision #3 extended again) — the engine's new scheduler needs to audit expired assignments identically to how `api`'s lifecycle/reassign/unassign paths already do, and `api` now needs to emit metrics (manual-override rate, lifecycle outcomes, order creation) that were previously engine-only. `engine/src/telemetry.ts` and `api/src/services/assignmentAudit.ts` (the old shim locations) were deleted outright once no callers remained, rather than kept as re-export shims.
- **Rate metrics (acceptance/rejection/manual-override) are modeled as a 0/1 value per event with `avg` aggregation** — an average of 0/1s over a time window *is* the rate, so no new "ratio of two counters" aggregation type was introduced. `shared/src/telemetry/assignmentOutcomeMetrics.ts` centralizes the accept/reject/expire branching since both `api` (accept/reject) and `engine` (expiry sweep) need the identical logic.
- **`metric_points`'s new composite primary key `(id, ts)`** (migration 0022) is a genuine Postgres requirement — a partitioned table's PK must include the partition column — and a deliberate one-off departure from every other table's plain-UUID PK; safe because `MetricPoint` is insert-only and never looked up by bare `id`. The migration drops and recreates the table (no real data existed anywhere yet) and pre-creates the current month + 3 future partitions plus a `DEFAULT` catch-all, so no telemetry write ever fails waiting on the engine scheduler's first tick.
- **New engine scheduler** (`engine/src/scheduler/{index,expirySweep,gaugeSampler,partitionMaintenance}.ts`), wired into `engine/src/index.ts` next to the existing heartbeat/consumer, each on its own `setInterval` cadence: `SLA_SWEEP_INTERVAL_MS` (default 10s), `GAUGE_SAMPLE_INTERVAL_MS` (default 30s), `PARTITION_MAINTENANCE_INTERVAL_MS` (default 86400000 / daily). `expirySweep.ts` uses the ORM's `lock` + `skipLocked` (the Sequelize-level equivalent of `shared/src/queue/claim.ts`'s raw `SKIP LOCKED` claim) so multiple engine instances never double-expire the same assignment.
- **The scheduler is a new autonomous writer of `assignments` rows, which exposed a latent gap**: `api/src/services/lifecycleService.ts` and `assignmentService.ts` (`reassignOrder`/`unassignOrder`) previously read the active `Assignment` row with a plain unlocked query before updating it — harmless when the only other writer was the equally-transactional `assigner.ts`, but capable of silently overwriting a concurrent expiry once the scheduler exists. All three now lock the row (`SELECT ... FOR UPDATE`) before transitioning it, matching `assigner.ts`'s and `expirySweep.ts`'s own recheck-under-lock discipline — a blocked lookup re-evaluates its `WHERE` clause against the post-commit row once unblocked, so a row the sweep already moved out of `ACTIVE_ASSIGNMENT_STATES` correctly stops matching instead of being overwritten. A dedicated multi-instance stress test for this specific race is deferred to Phase 6 alongside the existing "multi-instance engine tests (no double-dispatch)" roadmap item, per this codebase's established phase-alignment convention for concurrency tests (see Phase 0's notes on the same pattern).
- **`worker.active_count`/`worker.idle_count` semantics**, per their `metric_definitions` descriptions: *active* = any worker with ≥1 active assignment (regardless of its `status` column); *idle* = `status = "available"` **and** zero active assignments. A `busy`/`offline` worker with zero active assignments counts as neither — it's not meaningfully idle capacity. `worker.utilization` excludes workers whose effective capacity resolves to `Infinity` (nothing set anywhere in the Settings cascade), since dividing by `Infinity` would be meaningless.
- **`GET /metrics/query`** (`shared/src/telemetry/queryMetrics.ts`) uses raw `sequelize.query` (this codebase's established idiom for non-trivial SQL — see `eligibility.ts`), with `groupBy`/aggregation SQL fragments selected only from fixed whitelists (a Zod enum at the route layer; the metric's own validated `aggregation` column) and never string-interpolated from request text. The aggregation function is always the one configured on the `metric_definitions` row — a caller can't accidentally average a metric defined as a `p95`.
- **Custom metric definitions (`POST /metrics/definitions`) are deliberately not audited** — `AuditLog.entity`'s union (`"setting" | "pipeline_config" | "assignment"`) stays closed. Metric dictionary rows are descriptive metadata, not operational config that changes dispatch behavior, matching this doc's existing audit scope exactly.

**Phase 5 — Interface** ✅ Done
...
- Windows/Linux start-stop scripts (`scripts/{windows,linux}_{start,stop}.{ps1,sh}`) that bring up Postgres, run migrations, and launch all three Node services for local dev.

**Implementation notes (for later phases):**
- **New `interface/` workspace**: Next.js 16.3.0 (App Router, Turbopack), React 19.2.8, TypeScript 7 — native ESM like every other workspace. Tailwind v4's CSS-first `@theme inline` config (no `tailwind.config.js`) drives the design system from `app/globals.css`; SWR 2.5.0 for client data fetching (`lib/hooks.ts`'s `LIVE_POLL_MS = 5000` polling config on telemetry-ish hooks); Recharts 3.10.1 for charts; MapLibre GL 6.2.0 + free OpenStreetMap raster tiles (no API key/account needed) for the Command Dashboard's active-assignments map.
- **Deviation from this section's original Server/Client split**: every screen under `interface/app/` is a Client Component, not just the "live telemetry" ones — simplicity and one consistent data-fetching path (SWR hooks everywhere) won out over splitting cold CRUD screens (Groups, Metrics Dictionary) into Server Components. Revisit if first-paint latency on those screens becomes a real complaint.
- **`lib/api.ts` / `lib/hooks.ts` / `lib/scope.tsx`**: one typed fetch wrapper (`lib/api.ts`) parses the API's `{ error: { message, code, details } }` envelope into an `ApiError`; one SWR hook per resource lives in `lib/hooks.ts`; a `ScopeProvider` (`lib/scope.tsx`) persists the acting-dispatcher name plus the selected group/jurisdiction in `localStorage` and feeds the former into every mutating request's `X-Actor` header (Phase 1's actor convention) — there's still no auth, so this is the only source of a stable "who did this" value in audit trails.
- **Two API gaps surfaced by building against the real contract (flagged, not fixed here):** (1) there's still no `GET /settings/effective` endpoint (noted as a gap back in Phase 1) — the Settings screen resolves the jurisdiction→group→global cascade client-side by fetching all three scopes and merging in the browser; (2) there's no endpoint at all for `zone_workers` (worker↔zone coverage) CRUD, so the Workers screen surfaces schedule/status/skills but has no way to assign a worker to a zone — needs a new `api/` route to close.
- **Root-cause fix in already-shipped code, not scope creep:** `api/src/server.ts`, `engine/src/config.ts`, and `shared/src/db/sequelize.ts` each did a bare `import "dotenv/config"`, which resolves `.env` against `process.cwd()` — harmless for the Vitest/testcontainers suite (which passes `DATABASE_URL` explicitly) but broken for the real `npm run migrate` / `npm run dev` flow this phase's start scripts are the first thing to actually exercise end-to-end: npm sets `cwd` to the workspace directory for a `--workspace=X` script, so a repo-root `.env` was invisible to all three. Fixed by resolving `.env`'s path explicitly against each file's own location via `import.meta.url` (stable across `src/` under `tsx` and `dist/` under `node`, since `tsc` mirrors `src/`'s folder depth into `dist/`).
- **New top-level `scripts/` directory** (not an npm workspace). Both start scripts bring up Postgres via `docker compose`, wait for its healthcheck, run migrations (idempotent — safe to run on every start), then launch `api`/`engine`/`interface` as tracked background dev processes (PIDs in `.pids/`, logs in `logs/`, both gitignored). Windows-specific gotcha: `Start-Process -FilePath "npm"` fails because `npm` resolves to `npm.cmd`, not a launchable Win32 executable, so `windows_start.ps1` shells through `cmd.exe /c`; `windows_stop.ps1` then has to walk and kill the whole descendant process tree (`cmd.exe → npm-cli.js → tsx`/`next`, several hops from the one tracked PID) rather than just that PID. `linux_start.sh` instead launches each service via `setsid` so it gets its own process group, letting `linux_stop.sh` kill the whole tree with one `kill -- -PID`. Both stop scripts `docker compose stop` (not `down`) Postgres, so the container and its data volume are preserved, not removed.
- Root `package.json` workspaces now include `interface`; the interface dev server runs on port 3001 (`next dev -p 3001`) to avoid colliding with the API's 3000; root `.env.example` gained `PORT` and `NEXT_PUBLIC_API_BASE_URL`.
- **Pipeline Editor reorders stages via up/down buttons, not pointer drag-and-drop** — a deliberate simplification to avoid a DnD library dependency. The reordering/toggling *behavior* this doc calls for is fully implemented, just not with drag interaction.
- **The dashboard's chart palette is a re-derived, six-checks-validated palette, not the dataviz skill's generic reference palette.** The categorical/sequential/diverging colors are validated derivations (lightness band, chroma floor, CVD separation, normal-vision floor, contrast — checked against both the light `#F7EDE5` and dark `#050533` chart surfaces) of this doc's own brand hexes — Betel Orange, Procyan Green, Vega Blue, Aldebaran Yellow, Canopus Violet, Arcturus Red (the darker two of those six re-stepped in lightness only, hue unchanged, to clear the validator's band) — not the skill's off-the-shelf blue/orange/aqua/... set. Status colors (good/warning/serious/critical) use the skill's fixed reserved steps unchanged, per its own "status is never themed" rule.
